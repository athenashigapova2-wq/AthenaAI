...
export default function MacroCalculator() {
  ...
  useEffect(() => {
    (async () => {
      if (!user?.id) return;
      const profiles = await entities.UserProfile.filter({ created_by_id: user.id });
      if (profiles.length) {
        const p = profiles[0];
        setProfile(p);
  ...
  return (
    <div className="px-4 pt-6 pb-6 space-y-5">
      <header>
        ...
        <h1 className="text-2xl font-heading mt-1">{t("calc_title")}</h1>
      ...
      {result && (
        <>
              ...
              {VARIANTS.map((v) => {
                ...
                return (
                  <button
                    ...
                    className={`text-left rounded-2xl p-3 border-2 transition-colors ${active ? "border-primary bg-primary/5" : "border-border bg-card hover:border-primary/40"}`}
                  >
                    <span className="text-xl">{v.emoji}</span>
                    <p className="text-sm font-heading mt-1">{t(v.labelKey)}</p>
                    <p className="text-[11px] text-muted-foreground">{t(v.subKey)}</p>
                    <p className="text-sm font-semibold mt-1">{cal} {t("calc_kcal")}</p>
                  </button>
                );
              })}
            </div>
          ...
          <div className="bg-card rounded-2xl border-2 border-primary/30 p-4 space-y-3">
              ...
              <div>
                ...
                {editing ? (
                  <div className="flex items-center gap-2 mt-1">
                    <Input type="number" value={manualCal ?? ""} onChange={(e) => setManualCal(parseInt(e.target.value) || 0)} className="h-9 w-32 text-lg font-heading" />
                    <Button size="icon" className="h-9 w-9" onClick={() => { setEditing(false); setApplied(false); setSelectedKey("custom"); }}>
            ...
            {profile && (
              <Button variant="outline" className="w-full h-10" onClick={saveToProfile} disabled={saving}>
...
function Stat({ label, value, highlight }) {
  return (
    <div className={`rounded-xl p-3 ${highlight ? "bg-primary/10 border border-primary/30" : "bg-muted/50"}`}>
